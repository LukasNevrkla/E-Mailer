...
namespace E_Mailer
{
    ...
    /// </summary>
    public class AnimateSlideInFromDownProperty : AnimateBaseProperty<AnimateSlideInFromDownProperty>
    {
        protected override async void DoAnimation(FrameworkElement element, bool value)
        {
            if (value)
                // Animate in
                await (element as BasePage).PageBasicsStartEndAnimation(1, true);
                //await element. SlideAndFadeInFromLeftAsync(FirstLoad ? 0 : 0.3f, keepMargin: false);
            else
                // Animate out
                await (element as BasePage).PageBasicsStartEndAnimation(1, false);
